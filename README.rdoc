...
There are Minitest tests for the matrix generation, as the data requirements made it easier to do in the database. Running thes tests requries a database. There is a test.dump in the root which is needed for the tests.

Update the test database if you add more tables
    $ pg_dump -U [USERNAME] maturity_matrix_db_development -Fc > test.dump

Restore the test database
    $ pg_restore --verbose --clean --no-acl --no-owner -h localhost -U [USERNAME] -d maturity_matrix_db_test test.dump

Run the Minitest tests
    $ rake test

There are RSpec tests for user management (devise). Run these using rake spec
    $ rake spec

=Local developemnt

The application requires the database to contain the Digital Maturity Matrix structure in order to work properly. You can simply use the test database to populate the dev database:

	pg_restore --verbose --clean --no-acl --no-owner -h localhost -U [USERNAME] -d maturity_matrix_db_development test.dump
...
For Heroku, once the app is up and running and has postgres added, host the test database somewhere that can be accessed by a standard http request (such as amazon s3) and use:

heroku pg:backups restore [WEB URL FOR DATABASE] DATABASE_URL
