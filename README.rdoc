...
To interact with the db via command line if running postgres app:
    ...
    \connect maturity_matrix_db_development 
    # list data tables
    \dt 
    # quit
    \q
...
There are Spec tests for the matrix generation, as the data requirements made it easier to do in the database. Running thes tests requries a database. There is a test.dump in the root which is needed for the tests.
